...
from lib.layers.pooling import MAC, SPoC, GeM, GeMmp, RMAC, Rpool
from lib.layers.normalization import L2N, PowerLaw
...
def init_network(params):
    ...
    if teacher == 'resnet101':
        dim = OUTPUT_DIM_RESNET[architecture]
    else:
        dim = OUTPUT_DIM[architecture]

    # loading network from torchvision
    if pretrained:
        if architecture not in FEATURES and architecture != 'mobilenet_v3' and architecture != 'efficientnet_b3' and architecture != 'efficientnet_b3_new':
            ...
            net_in = timm.create_model('mobilenetv3_100', num_classes = 1000, in_chans = 3, pretrained=True, checkpoint_path='')
        elif architecture == 'efficientnet_b3':
            net_in = timm.create_model('tf_efficientnet_b3', num_classes = 1000, in_chans = 3, pretrained=True, checkpoint_path='')
        elif architecture == 'efficientnet_b3_new':
            net_in = EfficientNet.from_pretrained('efficientnet-b3') 
        else:
        ...
        if architecture != 'mobilenet_v3' and architecture != 'efficientnet_b3':
            net_in = getattr(torchvision.models, architecture)(pretrained=False)
        elif architecture == 'efficientnet_b3':
            net_in = timm.create_model('tf_efficientnet_b3', num_classes = 1000, in_chans = 3, pretrained=False, checkpoint_path='')
        else:
            net_in = timm.create_model('mobilenetv3_100', num_classes = 1000, in_chans = 3, pretrained=False, checkpoint_path='')
    #pdb.set_trace()
    ...
    if architecture.startswith('alexnet'):
        ...
        features = net_in.features
        features = nn.Sequential(features, nn.Conv2d(1280, dim, kernel_size=(1, 1), stride=(1, 1), bias=False).cuda())
        features = list(features.children())
        features.append(nn.ReLU(inplace=True))
    elif architecture.startswith('mobilenet_v3'):
        features = list(net_in.children())[:-2]
        ...
        features = list(net_in.children())[:-4]
        features.append(nn.Conv2d(1536, 512, kernel_size=(1, 1), stride=(1, 1), bias=False))
        features.append(nn.ReLU(inplace=True))
    else:
...
def extract_vectors(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, workers=8):
